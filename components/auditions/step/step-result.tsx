...
export function StepResult({ data }: StepResultProps) {
  ...
  return (
    <div className="w-full max-w-6xl font-sans mx-auto space-y-6 overflow-y-auto max-h-[75vh] custom-scrollbar pr-2 pb-12">
      {/* Introductory block */}
      {data.intro && (
            ...
            <ul className="space-y-5">
              {section.items.map((item, i) => (
                <li key={i} className="flex items-start text-[#B7BCB6] text-sm md:text-base leading-relaxed">
                  {/* Orange Bullet Point */}
                  <span className="mr-3 text-[#FF7316] mt-0.5 opacity-70 shrink-0">•</span>
